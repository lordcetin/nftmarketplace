...
import { sepnft,sepresell,seprpc,septoken,sepmarket,sepauction,cipherSep} from '../engine/configuration';
...
const PersonalAuctions = ({param,myauctionarr}) => {

  const {user,getUser,connectUser,auction,cri,seprpc} = useStateContext();
  ...
  const [myauction,MumsetAuction] = useState([]);
  ...
  useEffect(() => {
    if(!user.length || !window?.ethereum?.selectedAddress){
      connectUser()
    }else{
      setAddress(user)
    }
...
{/*const handleBidSubmit = async (e,tokenId) => {
  e.preventDefault()
  const web3Modal = new Web3Modal()
  const connection = await web3Modal.connect()
  const provider = new ethers.providers.Web3Provider(connection)
            ...
            ? <AudioPlayer nft={i.images} nftcover={i.cover} nftname={i.name}/> : null
              ...
              <div className='flex justify-between items-center w-full my-3'>
                <div className="flex justify-start items-center w-full">
                  <h3 className="text-md font-medium text-purple-500">{i.owner == user ? 'Created by You' : `${i.username}`}</h3>
                </div>
